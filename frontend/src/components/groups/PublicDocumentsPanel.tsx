...
export function PublicDocumentsPanel({ documents, allowDownloads, slug }: PublicDocumentsPanelProps) {
  return (
    <div className='col-span-4 border-l-2 border-neutral-950 bg-neutral-50 p-6 flex flex-col gap-4 overflow-y-auto'>
      ...
      {documents && documents.length > 0 ? (
        <div className='flex flex-col gap-3'>
          {documents.map(doc => (
            <div 
              key={doc.id} 
              className='bg-white border-2 border-neutral-950 p-4 rounded shadow-hard-sm flex items-center justify-between hover:translate-x-px hover:translate-y-px hover:shadow-none transition-all cursor-pointer'
            >
