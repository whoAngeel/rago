...
const statusConfig = {
  ...
  processing: {
    label: "Procesando",
    bg: "bg-accent-blue/20 animate-pulse",
    dot: "bg-blue-700",
  },
  completed: {
    label: "Completado",
    bg: "bg-primary-400/30",
    dot: "bg-primary-500",
  },
  failed: {
    label: "Fallido",
    bg: "bg-accent-red/30",
    dot: "bg-accent-red-deep",
  },
};
...
const getFileIcon = (contentType: string) => {
  ...
  if (
    ...
    return <FileSpreadsheet size={20} className="text-black" />;
...
function StatusBadge({ doc }: { doc: Document }) {
  ...
  return (
    <div className="flex flex-col gap-1 items-start">
      <span
        className={`px-3 py-1 ${isRetrying ? "bg-amber-100" : cfg.bg} border border-neutral-950 rounded-md text-xs font-bold uppercase flex items-center gap-1.5 w-fit shadow-hard-sm`}
      >
        {isRetrying ? (
        ...
        createPortal(
          ...
          >
            <div className="px-2 py-1 bg-neutral-100 border-b-2 border-neutral-950 flex items-center justify-between">
              <span className="text-[10px] font-bold uppercase tracking-wider text-neutral-950">
                Pipeline
              </span>
              {doc.retry_count > 0 && (
  ...
  return (
    ...
    >
      <div className="flex items-start gap-3">
        <div
          className={`w-10 h-10 ${getFileIconBg(document.content_type)} border-2 border-neutral-950 rounded-[var(--radius-btn)] flex items-center justify-center shadow-[var(--shadow-hard-sm)] shrink-0`}
        >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-neutral-500 font-medium">
            {getFileTypeLabel(document.content_type)}
          </p>
        </div>
      </div>
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-1">
          ...
          <button
            className="p-2 text-neutral-600 hover:text-accent-red-deep border-2 border-transparent hover:border-neutral-950 hover:bg-accent-red/10 hover:shadow-hard-sm rounded transition-all cursor-pointer"
            title="Eliminar"
      ...
      <div className="block md:hidden space-y-3 p-4">
        {documents.length === 0 && (
          <div className="flex flex-col items-center justify-center gap-3 py-12">
            ...
            <div>
              <p className="font-bold text-neutral-950 text-lg">
                No hay documentos aún
              </p>
              <p className="text-sm text-neutral-500">
                Sube un archivo para empezar a procesarlo
              </p>
            </div>
          </div>
        )}
                ...
                >
                  ...
                  <td className="px-6 py-5">
                    <div className="flex items-center gap-2 justify-center">
                      ...
                      <button
                        className="p-2 text-neutral-600 hover:text-accent-red-deep
