...
func NewQdrantStore(host string, port int) (*QdrantStore, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("error creando cliente Qdrant: %w", err)
	}
...
func (s *QdrantStore) CreateCollection(ctx context.Context, name string, size int) error {
	collections := s.client.GetCollectionsClient()
...
func (s *QdrantStore) UpsertDocuments(ctx context.Context, collection string, docs []schema.Document, vectors [][]float32) error {
...
func (s *QdrantStore) Search(ctx context.Context, collection string, queryVector []float32, limit int) ([]schema.Document, error) {
	pointsClient := s.client.GetPointsClient()
...
func (s *QdrantStore) GetPointsCount(ctx context.Context, collection string) (uint64, error) {
	collections := s.client.GetCollectionsClient()
